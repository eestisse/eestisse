...
type alias PostmarkSend =
    ...
    , to : List { name : String, email : EmailAddress }
    , subject : String
...
sendEmailTask : PostmarkSend -> Task Http.Error PostmarkSendResponse
sendEmailTask d =
    let
        httpBody =
            Http.jsonBody <|
    ...
    Http.task
        { method = "POST"
        , headers = [ Http.header "X-Postmark-Server-Token" Env.postmarkToken ]
        , url = endpoint ++ "/email"
        , body = httpBody
...
sendEmail :
    ...
    -> Cmd msg
sendEmail msg d =
    sendEmailTask d |> Task.attempt msg


emailsToString : List { name : String, email : EmailAddress } -> String
